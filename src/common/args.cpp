...
const char * const BITCOIN_CONF_FILENAME = "bitcoinknotscash.conf";
...
fs::path GetDefaultDataDir()
{
    ...
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinKnotsCash";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "BitcoinKnotsCash";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/BitcoinKnotsCash";
#else
    // Unix-like
    return pathRet / ".bitcoinknotscash";
#endif
#endif
}
